...
class Book {
  constructor(title, language, author, pages, read) {
    this.title = title;
  ...
  addBookDiv() {
    for (let i = 0; i < myLibrary.length; i += 1) {
      ...
      btnRead.addEventListener('click', () => {
        ...
        const indexToFind = myLibrary.find(({ title }) => title === thisBookTitle);
        const indexBookTitle = myLibrary.indexOf(indexToFind);
